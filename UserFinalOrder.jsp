...
Double totalPrice = Double.parseDouble((String)total);
...
<tr><td>SI NO</td><td>Product</td><td>Price $</td></tr>
<%//started the iterator and display information in the vector 
while(itId.hasNext()){%>
<tr><td><%out.print(itId.next());%></td><td><%out.print(itName.next());%></td>
<td><%out.print(fmt.format(itPrice.next()));%></td>
<td><input type="checkbox" name = "buyList" value='<%=num %>'></td> </tr>
<%num++; %>
<% }%>
</table><br><br>
<h2>Total Items:<%out.print(productId.size()); %></h2>
<h2>Total Price:<%out.print(fmt.format(totalPrice)); %></h2>
</center>
